...
void cli_biz(int connfd,int cid){
    while (1)
    {
        ...
        // 创建一个 PDU
        int len = strlen(buf);
        char *pduSend = (char*)malloc(2 + len);
        // 消息头为2字节的CID
        ...
        write(connfd, pduSend, 2 + len);
        // 读取 server 回射的PDU头部的两字节VCD
        short vcd;
        read(connfd, &vcd, 2);
        // 读取 server 回射的PDU体
        char pduRecv[1024];
        read(connfd, pduRecv, vcd);
        // 将 server 回射的PDU体输出到 stdout
        // 格式为`[cli](pid)[vcd](vcd)[ECH_REP] <msg>`
        printf("[cli](%d)[vcd](%d)[ECH_REP]%s\n", getpid(), ntohs(vcd), pduRecv);
        // 释放内存
        free(pduSend);
        free(pduRecv);
    }
